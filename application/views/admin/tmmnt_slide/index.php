...
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">메인 호전사례 슬라이드 관리</h3>
    </div>
    <!-- /.col-lg-12 -->
</div>
...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-default">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:right;">
                        ...
                        <label>
                            <select class="form-control input-sm" name="search_field" id="search_field" aria-controls="dataTables-example">
                                ...
                                <option value="TI_TTL" <?=($search_field == "TI_TTL") ? 'selected': ''?>>제목</option>
                                <option value="TI_NM" <?=($search_field == "TI_NM") ? 'selected': ''?>>환자성명</option>
                            </select>
                        </label>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <?foreach($result as $rows):?>
                            <tr class="odd gradeX">
                                ...
                                <td class="text-center"><a href="/admin/tmmnt_slide/regist?TI_SEQ=<?=$rows["TI_SEQ"]?>"><?=$rows['TI_TTL']?></a></td>
                                <td class="text-center"><?=$rows['TI_NM']?></td>
                                <td class="text-center"><? if($rows['TI_EXPS_YN'] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-default">미노출</span><? }?></td>
                                <td class="text-center"><? if($rows['TI_EXPS_YN'] == "Y"){ ?><?=$rows['TI_SORT']?><? } ?></td>
                                <td class="text-center"><?=substr($rows['TI_RGST_YMDT'],0,10)?></td>
                            </tr>
                            <?$cur_num--?>
                            <?$row_cnt++;?>
                        <?endforeach?>
            ...
            <div class="panel-heading" style="height:55px;">
                <button type="button" class="btn btn-success pull-right" onclick="location.href='/admin/tmmnt_slide/regist';">등록</button>
            </div>
        </div>
        <!-- /.panel -->
