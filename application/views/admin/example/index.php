...
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">호전사례 발표</h3>
    </div>
    <!-- /.col-lg-12 -->
</div>
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:right;">
                        ...
                        <label>
                            <select class="form-control input-sm" name="search_field" id="search_field" aria-controls="dataTables-example">
                                ...
                                <option value="PT_TTL" <?=($search_field == "PT_TTL") ? 'selected': ''?>>제목</option>
                                <option value="PT_CONT" <?=($search_field == "PT_CONT") ? 'selected': ''?>>내용</option>
                            </select>
                        </label>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <?foreach($result as $rows):?>
                            <tr class="odd gradeX">
                                ...
                                <td class="text_center"><a href="/admin/example/regist?PT_SEQ=<?=$rows["PT_SEQ"]?>"><?=$rows['PT_TTL']?></a></td>
                                <td class="text-center"><? if($rows['PT_EXPS_YN'] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-info">미노출</span><? }?></td>
                                <td class="text-center"><?=$rows['PT_READ_CNT']?></td>
                                <td class="text-center"><?=$rows['PT_RPRT_YMD']?></td>
                                <td class="text-center"><?=substr($rows['PT_RGST_YMDT'],0,10)?></td>
                            </tr>
                            <?$cur_num--?>
                            <?$row_cnt++;?>
                        <?endforeach?>
            ...
            <div class="panel-footer" style="height:55px;">
                <button type="button" class="btn btn-success pull-right" onclick="location.href='/admin/example/regist'">등록</button>
            </div>
        </div>
        <!-- /.panel -->
