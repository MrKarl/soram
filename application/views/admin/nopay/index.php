...
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">비급여 관리</h3>
    </div>
    <!-- /.col-lg-12 -->
</div>
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:right;">
                        ...
                        <label>
                            <select class="form-control input-sm" name="search_field" id="search_field" aria-controls="dataTables-example">
                                ...
                                <option value="name" <?=($search_field == "name") ? 'selected': ''?>>명칭</option>
                                <option value="spec" <?=($search_field == "spec") ? 'selected': ''?>>비고</option>
                            </select>
                        </label>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        <thead>
                            <tr>
                                ...
                                <th style="width:50%;" class="text-center">명칭</th>
                                <th style="width:10%;" class="text-center">금액</th>
                        ...
                        <tbody>
                            <?foreach($result as $rows):?>
                                <tr class="odd gradeX">
                                    ...
                                    <td class="text_center"><a href="/admin/nopay/regist?idx=<?=$rows["idx"]?>"><?=$rows['name']?></a></td>
                                    <td class="text-center"><?=number_format($rows["price"]);?></td>
                                    <td class="text-center"><?=$rows["spec"]?></td>
                                    <td class="text-center"><? if($rows["exps_yn"] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-danger">미노출</span><? } ?></td>
                                    <td class="text-center"><?=substr($rows["reg_date"],0,10)?></td>
                                </tr>
                                <?$cur_num--?>
                                <?$row_cnt++;?>
                            <?endforeach?>
            ...
            <div class="panel-footer" style="height:55px;">
                <button type="button" class="btn btn-success pull-right" onclick="location.href='/admin/nopay/regist'">등록</button>
            </div>
        </div>
        <!-- /.panel -->
