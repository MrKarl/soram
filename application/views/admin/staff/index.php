...
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">전문 의료진 관리</h3>
    </div>
    <!-- /.col-lg-12 -->
</div>
...
<div class="row">
    <div class="col-lg-12">
        <div class="panel panel-default">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:right;">
                        ...
                        <label>
                            <select class="form-control input-sm" name="search_field" id="search_field" aria-controls="dataTables-example">
                                ...
                                <option value="STF_NM" <?=($search_field == "STF_NM") ? 'selected': ''?>>이름</option>
                                <option value="STF_JOB" <?=($search_field == "STF_JOB") ? 'selected': ''?>>업무</option>
                            </select>
                        </label>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        <thead>
                            <tr>
                                ...
                                <th style="width:30%;" class="text-center">구분</th>
                                <th style="width:20%;" class="text-center">노출여부</th>
                                <th style="width:20%;" class="text-center">등록일</th>
                            </tr>
                        </thead>
                        <tbody>
                        <?foreach($result as $rows):?>
                            <tr class="odd gradeX">
                                ...
                                <td class="text-center"><a href="/admin/staff/regist?STF_SEQ=<?=$rows["STF_SEQ"]?>"><?=$rows['STF_NM']?></a></td>
                                <td class="text-center"><?=($rows['CC_NM'] != "") ? $rows['CC_NM'] : "없음"?></td>
                                <td class="text-center"><? if($rows['STF_EXPS_YN'] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-default">미노출</span><? }?></td>
                                <td class="text-center"><?=substr($rows['STF_RGST_YMDT'],0,10)?></td>
                            </tr>
                            <?$cur_num--?>
                            <?$row_cnt++;?>
                        <?endforeach?>
            ...
            <div class="panel-heading" style="height:55px;">
                <button type="button" class="btn btn-success pull-right" onclick="location.href='/admin/staff/regist';">등록</button>
            </div>
        </div>
        <!-- /.panel -->
