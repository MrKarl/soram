    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:left;">
                        <label>
                            <select class="form-control input-sm pull-left" name="is_exps" id="is_exps" aria-controls="dataTables-example" onchange="searchStatus()">
                                <option class="txt" value="" <?=($is_exps == "") ? 'selected': ''?>>노출현항(전체)</option>
                                <option class="txt" value="Y" <?=($is_exps == "1") ? 'selected': ''?> >노출</option>
                                <option class="txt" value="N" <?=($is_exps == "2") ? 'selected': ''?> >미노출</option>
                            </select>
                        </label>
                    </div>
                    <div class="dataTables_filter" style="display: inline-flex; float:left;">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:right;">
                        ...
                        <label>
                            <select class="form-control input-sm" name="search_field" id="search_field" aria-controls="dataTables-example">
                                ...
                                <option value="PP_TTL" <?=($search_field == "PP_TTL") ? 'selected': ''?>>제목</option>
                                <option value="PP_CONT" <?=($search_field == "PP_CONT") ? 'selected': ''?>>내용</option>
                            </select>
                        </label>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        <thead>
                            <tr>
                                ...
                                <th style="width:20%;" class="text-center">게시기간</th>
                        ...
                        <?foreach($result as $rows):?>
                            <tr class="odd gradeX">
                                ...
                                <td class="text_center"><a href="/admin/popup/regist?PP_SEQ=<?=$rows["PP_SEQ"]?>"><?=$rows['PP_TTL']?></a></td>
                                <td class="text-center"><?=$rows['PP_STRT_YMD']?> ~ <?=$rows['PP_END_YMD']?></td>
                                <td class="text-center"><? if($rows['PP_EXPS_YN'] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-info">미노출</span><? }?></td>
                                <td class="text-center"><?=substr($rows['PP_RGST_YMDT'],0,10)?></td>
                            </tr>
                            <?$cur_num--?>
                            <?$row_cnt++;?>
                        <?endforeach?>
            ...
            <div class="panel-footer" style="height:55px;">
                <button type="button" class="btn btn-success pull-right" onclick="location.href='/admin/popup/regist'">등록</button>
            </div>
        </div>
        <!-- /.panel -->
