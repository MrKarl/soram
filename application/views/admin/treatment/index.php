    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
                    ...
                    <div class="dataTables_filter" style="display: inline-flex; float:right;">
                        ...
                        <label>
                            <select class="form-control input-sm" name="search_field" id="search_field" aria-controls="dataTables-example">
                                ...
                                <option value="TT_TTL" <?=($search_field == "TT_TTL") ? 'selected': ''?>>제목</option>
                                <option value="TT_NM" <?=($search_field == "TT_NM") ? 'selected': ''?>>환자성명</option>
                            </select>
                        </label>
                    ...
                    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        <thead>
                            <tr>
                                ...
                                <th style="width:10%;" class="text-center">암구분</th>
                                <th style="width:30%;" class="text-center">제목</th>
                                <th style="width:10%;" class="text-center">환자성명</th>
                                <th style="width:10%;" class="text-center">메인 노출여부</th>
                        ...
                        <?foreach($result as $rows):?>
                            <tr class="odd gradeX">
                                ...
                                <td class="text-center"><?=($rows['CC_NM']) ? $rows['CC_NM'] : "기타"?></td>
                                <td class="text_center"><a href="/admin/treatment/regist?TT_SEQ=<?=$rows["TT_SEQ"]?>"><?=$rows['TT_TTL']?></a></td>
                                <td class="text-center"><?=$rows['TT_NM']?></td>
                                <td class="text-center"><? if($rows['TT_MAIN_EXPS_YN'] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-info">미노출</span><? }?></td>
                                <td class="text-center"><? if($rows['TT_EXPS_YN'] == "Y"){ ?><span class="label label-success">노출</span><? }else{ ?><span class="label label-info">미노출</span><? }?></td>
                                <td class="text-center"><?=$rows['TT_READ_CNT']?></td>
                                <td class="text-center"><?=substr($rows['TT_RGST_YMDT'],0,10)?></td>
                            </tr>
                            <?$cur_num--?>
                            <?$row_cnt++;?>
                        <?endforeach?>
            ...
            <div class="panel-footer" style="height:55px;">
                <button type="button" class="btn btn-success pull-right" onclick="location.href='/admin/treatment/regist'">등록</button>
            </div>
        </div>
        <!-- /.panel -->
