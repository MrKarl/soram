            ...
            <table class="st_table1 mt10">
                ...
                <?if( count($result) > 0 ):?>
                    <?foreach($result as $rows):?>
                        <tr>
                            <td><?=$cur_num?></td>
                            <td><div class="st_oneline"><a href="./story_view?seq=<?=$rows["ST_SEQ"]?>"><?=$rows['ST_TTL']?><?if( floor((strtotime(date("Y-m-d"))-strtotime($rows['ST_RGST_YMDT']))/(60*60)) <= 52 ) {?><span class="icon_new">N</span><?}?></a></div></td>
                            <td class="write_date"><?=substr($rows['ST_RGST_YMDT'],0,10)?></td>
                        </tr>
                        <?$cur_num--?>
                        <?$row_cnt++?>
                    <?endforeach?>
