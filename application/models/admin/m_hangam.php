...
class M_hangam extends CI_Model{
    ...
    function selectHangamCount(){
        ...
        $this->db->from('t_hangam');
        $this->total = $this->db->count_all_results();
        return  $this->total;
    }

    function selectHangam(){
        ...
        $query = $this->db->get('t_hangam', $this->limit , $this->offset );
        return $this->result_array = $query->result_array();
    }

    function selectHangamOne($BC_SEQ){

        $this->db->where('BC_SEQ', $BC_SEQ);

        $query  = $this->db->get('t_hangam');
        return $result  = $query->row();
    }

    function insertProcess($data){
        return $this->db->insert("t_hangam", $data);
    }

    function updateProcess($data, $BC_SEQ){
        $this->db->where("BC_SEQ", $BC_SEQ);
        return $this->db->update("t_hangam", $data);
    }

    function deleteProcess($BC_SEQ){
        $this->db->where("BC_SEQ", $BC_SEQ);
        return $this->db->delete("t_hangam");
    }
}